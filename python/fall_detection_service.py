...
def start_monitoring():
    ...
    try:
        ...
        if camera_type == 'webcam':
            # Suporta tanto índice quanto device ID
            try:
                camera_index = int(camera_url)
            except ValueError:
                # É um device ID string
...
def trigger_webhook(session_id: int, confidence: float, snapshot: Optional[bytes] = None):
    ...
    try:
        ...
        logger.info(f"Sending webhook for session {session_id}")
        ...
        if response.status_code == 201:
            logger.info(f"Webhook successful for session {session_id}: {response.json()}")
        else:
            logger.warning(f"Webhook returned {response.status_code}: {response.text}")

    except requests.exceptions.Timeout:
        logger.error(f"Webhook timeout for session {session_id}")
    except Exception as e:
        logger.error(f"Webhook error for session {session_id}: {str(e)}")

def calculate_fall_confidence(landmarks) -> tuple[bool, float]:
    """
    Calcular confiança de detecção de queda

    Retorna: (is_fallen, confidence_percentage)
    """
    try:
        # Pontos chave
        ...
        # Lógica de detecção
        is_horizontal = avg_diff < 0.15  # Corpo está horizontal
        is_level = shoulder_horizontality < 0.08  # Ombros nivelados

        # Calcular confiança (0-100%)
        if is_horizontal and is_level:
            # Quanto menor a diferença vertical, maior a confiança
...
def process_video():
    ...
    fall_threshold_frames = 60  # ~2 segundos a 30fps
    last_webhook_time = 0
    webhook_cooldown = 30  # Cooldown de 30s entre webhooks
        ...
        try:
            ...
            if results.pose_landmarks:
                ...
                if is_fallen:
                    ...
                    # Confirmar queda após threshold
                    if consecutive_fall_frames >= fall_threshold_frames:
                        ...
                        # Box vermelho ao redor
                        h, w = frame.shape[:2]
                        cv2.rectangle(frame, (0, 0), (w, h), (0, 0, 255), 10)
                else:
                    # Reset contador gradualmente
